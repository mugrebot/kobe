...
export default function SwapSetModal({ writeContracts, contracts, tx, modalUp, handleModalDown, setDetails, address, set, gasPrice, USDPrices, wethBalance }) {
  ...
  const handleApproveTokens = async () => {
    setApproving(true)
    await tx(writeContracts.WETH.approve(issuerAddress, _response.allowanceTarget && _response.sellAmount))
    setApproving(false)
  }
    ...
    buyTokenPercentageFee: '0.02',
     ...
     const handleIssuance = async () => {
      ...
      const newTx = {
        from: address,
        to: ZERO_EX_ADDRESS,
        value: indexAmount && utils.parseEther(`${indexAmount}`),
        data: _response.data,
      }

      console.log(newTx)

      await tx(newTx, gasPrice)
      setBuying(false)
    }
